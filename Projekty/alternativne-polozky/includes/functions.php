...
function insert_work(){
    global $connect;
    if($_POST['tahova']==''){$_POST['tahova']=0;}
    if($_POST['machine']==''){$_POST['machine']=0;}
	if($_POST['action_type_other']==''){$_POST['action_type_other']=0;}
	if($_POST['vyhovuje']==''){$_POST['vyhovuje']=1;}
	$smallOrderValue = isset($_POST['small_order']) ? 1 : 0;
    
	$insert_work_query = "INSERT INTO udrzba_strojov (os_c, action_type, machine, kontakt, small_order, q, vp, tahova, vyhovuje, action_type_other, poznamka, datum, cas, druh) 
    VALUES (".$_POST['os_c'].", ".$_POST['action_type'].", ".$_POST['machine'].", '".$_POST['kontakt']."', ".$smallOrderValue.", '".$_POST['q']."', '".$_POST['vp']."', '".$_POST['tahova']."', ".$_POST['vyhovuje'].", ".$_POST['action_type_other'].", '".$_POST['note']."', NOW(), '".$_POST['time']."', '".implode(",", $_POST['druh'])."')";
    if(mysqli_query($connect, $insert_work_query)){
        echo '<div class="alert alert-success">Práca bola zaznamenaná!</div>';
    }  
    else{
        echo '<div class="alert alert-danger">Práca nebola zaznamenaná!</div>';
        echo '<div class="alert alert-danger">query:'.$insert_work_query.'</div>';
    }
...
function deleteCommand(){
    ...
    $query="DELETE FROM kabelaze WHERE qty_r <= '0' AND datum_cas >= NOW() - INTERVAL 5";
    mysqli_query($connect,$query);
...
function checkModules($vp,$modul,$id){
    ...
    while(count($moduls)>$countOfModules){
        ...
        if($moduls[$countOfModules]!=""){
            ...
            $query="SELECT vp2, artikel, os_cislo_skladnik FROM moduly_presun WHERE vp2='".$vp."' ";
    ...
    if(!in_array(0,$modules_done)){
                ...
                $query_polozky="SELECT moduly_hotove, stav_vyroby FROM polozky WHERE id='".$id."' ";
                ...
                $stav_vyroby=$result_zaznamy_polozky['stav_vyroby']==-1?0:1;
                $query_zaznamy="UPDATE polozky SET moduly_hotove='1', moduly_updated=NOW(), moduly_person_id=".$majster.", stav_vyroby=stav_vyroby+".$stav_vyroby." WHERE id='".$id."'";       
...
function updateCommand(){
    ...
    if(isset($_POST['cd'])){
        $query="SELECT cd_hotove, stav_vyroby FROM polozky WHERE id='".$_POST['id_polozka']."' ";
        $apply = mysqli_query($connect,$query);
        $row = mysqli_fetch_assoc($apply);
         
        $stav=($_POST['cd']==1?1:-1);
        $cd_stav=$stav;
        $stav=$row['stav_vyroby']==-1?0:$stav;
        //$cd_stav=($row['stav_vyroby']==-1 && $row['cd_hotove']==-1)?-1:$cd_stav;
        if($stav!=$row['cd_hotove']){
           $query_zaznamy="UPDATE polozky SET cd_hotove='".$cd_stav."', cd_updated=NOW(), stav_vyroby=stav_vyroby+".$stav.", cd_person_id=".$_SESSION['username']." WHERE id='".$_POST['id_polozka']."'";       
            $apply_zaznamy=mysqli_query($connect,$query_zaznamy);
        }
        unset($_POST['cd']);
    ...
    if(isset($_POST['moduly'])){
        $query="SELECT moduly_hotove, stav_vyroby FROM polozky WHERE id='".$_POST['id_polozka']."' ";
        $apply = mysqli_query($connect,$query);
        $row = mysqli_fetch_assoc($apply);
        ...
        if($stav!=$row['moduly_hotove']){
            $query_zaznamy="UPDATE polozky SET moduly_hotove='".$modul_stav."', moduly_updated=NOW(), stav_vyroby=stav_vyroby+".$stav.", moduly_person_id=".$_SESSION['username']." WHERE id='".$_POST['id_polozka']."'";       
            $apply_zaznamy=mysqli_query($connect,$query_zaznamy);
        }
        unset($_POST['moduly']);
    ...
    if(isset($_POST['chyba'])){
        ...
        if($_POST['chyba']!=0){
            ...
            $query="UPDATE moduly_presun SET os_cislo_majster=0, datum_cas_majster='1970-01-01 00:00:00' WHERE vp2='".$_POST['vp']."'";
            $apply_zaznamy=mysqli_query($km,$query); 
        }
        unset($_POST['chyba']);
