<?php
	class User
	{
		...
		{
			$queryString = "select id from user where 1 AND username = '" . mysqli_real_escape_string($connect,$username) . "' and active = '1';";		
			$ResultQ = mysqli_query($connect,$queryString);
			if($ResultQ){//000
				if(mysqli_num_rows($ResultQ)==1){//001
					...
					//	overime si heslo
					$queryString = "select * from user where 1 and id = '" . $RowQ['id'] . "' and pwd = '" . mysqli_real_escape_string($connect,$passphrase) . "' limit 1;";
